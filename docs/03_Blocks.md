...
See [Locals](#locals-types-and-postfix-math) for what a local's type is otherwise, and
[CScalar](02_Shapes.md#jit_each-and-jit_map-when-nothing-reaches-a-neighbour) for what else one is good for.
...
- Block-local variables, assigned before use, reassignable -- including to a
  different type (see [Locals](#locals-types-and-postfix-math))
...
- the postfix spelling of those -- `x.sqrt`, `(0.0415 * (t[i] - 218.8)).tanh`
  -- which `CArray::CoreExtensions` provides (see [Postfix math](#postfix-math))
...
- a call to a C function -- one from `jit_extern` or `jit_function`, or the
  ...
  [Calling a C function](#a-call-may-stand-alone))
...
limitations](#known-limitations)), `if` without `else` in *expression*
...
- **A sine and a cosine of the same argument are one `sincos` call.** The C
  ...
  disabled; see [Design notes](05_DesignNotes.md#a-sine-beside-a-cosine-is-sincos-and-is-allowed-to-be).
