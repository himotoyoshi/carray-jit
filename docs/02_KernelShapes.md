...
Five things keep the driver here. An operand that is a **strided view**, for the reason just measured: it is walked in place here, and re-gathered there for nothing. A **stretched operand**, which is the one thing that cannot be flattened: broadcasting arrives as a stride of zero on an axis, and a flat run has no axes, so cell k of the output would stop lining up with cell k of the operand. A body that **asks about a mask**, because a chunk carries no per-cell mask to ask it of -- CArray ORs and propagates the masks, but `a[i] == UNDEF` is a question about one cell. A CArray **without the family** at all, which is asked for by symbol rather than assumed, so an older one falls back without the caller hearing about it. And a **gather from an array the pass also touches**: the sweep re-gathers what it cannot walk in place at the top of every chunk, so the cells it gathers have to stand still, and they do not where the pass writes the array they come from -- the second chunk would gather what the first chunk wrote. Ruby reads the whole right-hand side before assigning, which is what gathering once before the loop does. Sharing a root is the test, rather than overlapping cells: a view knows what it is a view of. A strided view of an array the pass writes is not kept out of the sweep but copied before the loop, for the same reason and to the same end -- `hi = lo * 2.0` over two blocks that share six cells reads what the expression started with, as Ruby's `hi[] = lo * 2.0` does, rather than the cells the pass has just written.
