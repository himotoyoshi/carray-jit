...
[Four ways](01_GettingStarted.md#four-ways-to-compute-an-expression-and-what-separates-them) --
...
[`jit_for`](#extents-steps-and-subscripts)'s:
...
- **A computed offset.** `a[k, 0]` where `k` is a value is refused: the
  ...
  inside. A subscript the kernel works out is [`jit_for`](#extents-steps-and-subscripts)'s,
...
- **Writing.** A stencil produces a value. A block that writes several arrays
  is [`jit_each`](#jit_each-and-jit_map-when-nothing-reaches-a-neighbour)'s.
