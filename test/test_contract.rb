...
class TestContract < Minitest::Test
  ...
  def test_the_parameter_order_gives_the_axis_order
    ...
    straight = CArray.contract { |i, j, k| a[i,k] * b[k,j] }
    swapped = CArray.contract { |j, i, k| a[i,k] * b[k,j] }
  ...
  def test_the_returned_form_with_nothing_free
    ...
    result = CArray.contract { |i, k| a[i,k] * b[i,k] }
  ...
  def test_the_returned_and_assigned_forms_agree
    a = CArray.double(3, 4).seq!(1)
    b = CArray.double(4, 2).seq!(1)
    returned = CArray.contract { |i, j, k| a[i,k] * b[k,j] }
    box = CArray.double(3, 2)
    CArray.contract { |i, j, k| box[i,j] = a[i,k] * b[k,j] }
  ...
  def test_matrix_times_vector
    ...
    CArray.contract { |i, k| result[i] = a[i,k] * v[k] }
  ...
  def test_a_full_contraction
    ...
    box = CArray.double(1)
    CArray.contract { |i, k| box[0] = a[i,k] * b[i,k] }
  ...
  def test_a_trace
    ...
    box = CArray.double(1)
    CArray.contract { |i| box[0] = square[i,i] }
  ...
  def test_an_index_summed_over_cannot_also_be_free
    ...
    error = assert_raises(CArray::JIT::Unsupported) do
      CArray.contract { |i, k| out[i] = a[i,k] * b[i,k] }
    end
  ...
  def test_an_index_that_names_no_axis
    ...
    out = CArray.double(3)
    error = assert_raises(CArray::JIT::Unsupported) do
      CArray.contract { |i, k, z| out[i] = a[i,k] * a[i,k] }
    end
  ...
  def test_an_index_appearing_more_than_twice
    ...
    out = CArray.double(1)
    error = assert_raises(CArray::JIT::Unsupported) do
      CArray.contract { |i| out[0] = a[i,i,i] }
    end
  ...
  def test_a_double_contraction
    ...
    CArray.contract { |i, j, k, l| result[i,j] = left[i,k,l] * right[l,k,j] }
    expected = (0...ni).map { |i|
      (0...nj).map { |j|
  ...
  def test_an_outer_product
    ...
    result = CArray.double(ni, nj)
    CArray.contract { |i, j| result[i,j] = left[i] * right[j] }
  ...
  def test_the_sum_starts_from_a_zero_of_the_summand_type
    ...
    CArray.contract { |i, j, k| result[i,j] = left[i,k] * right[k,j] }
    expected = (0...2).map { |i|
  ...
  def test_an_array_that_is_both_written_and_read
    ...
    error = assert_raises(CArray::JIT::Unsupported) do
      CArray.contract { |i, k| values[i] = values[k] * 2.0 }
    end
  ...
  def test_more_than_one_assignment
    ...
    error = assert_raises(CArray::JIT::Unsupported) do
      CArray.contract { |i, k| first[i] = a[i,k]; second[i] = a[i,k] }
    end
  ...
  def test_a_repeated_index_on_the_left
    ...
    error = assert_raises(CArray::JIT::Unsupported) do
      CArray.contract { |i, j| out[i,i] = a[i,j] * b[j,i] }
    end
  ...
  def test_a_contraction_per_row
    ...
    rows.times { |i| out[i] = CArray.contract { |k| a[i, k] * b[i, k] }[0] }
  ...
  def test_a_pinned_subscript_may_be_an_expression
    ...
    row = 1
    result = CArray.contract { |k| a[row + 1, k] * b[row + 1, k] }
  ...
  def test_a_pinned_position_outside_the_array
    ...
    row = 99
    error = assert_raises(CArray::JIT::Unsupported) do
      CArray.contract { |k| a[row, k] * b[row, k] }
    end
  ...
  def test_an_axis_may_be_walked_by_two_indices
    ...
    gram = CArray.contract { |a, b, p| c[p,a] * c[p,b] }
  ...
  def test_the_two_indices_may_be_summed_rather_than_free
    ...
    result = CArray.contract { |p, a, b| c[p,a] * m[a,b] * c[p,b] }
    expected = (0...4).sum { |p|
  ...
  def test_the_extents_of_both_indices_come_from_the_axis
    ...
    error = assert_raises(CArray::JIT::Unsupported) do
      CArray.contract { |a, b, p| c[p,a] * c[p,b] * weight[a,b] }
    end
    assert_match(/`b` addresses axes of different extents: /, error.message)
  end

  def test_a_block_is_required
    error = assert_raises(CArray::JIT::Unsupported) { CArray.contract }
